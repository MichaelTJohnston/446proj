...
nfixed = .5; % number orbits collecting science 
tau = (tSun/T); % fraction of period spent in the sun
ttx = (100000)/(dRateMax); % [s] time for data transfer
Esci_fixed = (nfixed*T) * (PgenFixed*tau - pModeSci); %  energy used in one orbit for data collection 
Etx_fixed = -ttx*pModeComms; % energy for transmitting data 
Egen_fixed = T * (orbits - nfixed - ttx)  * (PgenFixed45*tau  - pModeIdle); % energy generated by panels  

Eday_fixed = (Esci_fixed + Etx_fixed + Egen_fixed)/3600 % [W.hr] net energy gained/loss


ntrack = 1.25; % number of orbits collecting science 
ntx = ntrack * ((T*100000) / (tSun*dRateMax)); % 
