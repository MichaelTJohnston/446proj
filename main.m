...
effEoL = effPanel*(1 - effTemp*(Tsol - 28))*(1 - effTime)^opTime;  %end of life efficiency
syms t
%fore
t_enter_f = tEclipse/2; %time the fore panel enters light [s]
t_exit_f = T/2; %time for panel exits light [s]

fun_f = Fsun*APanel*effEoL*sin(w*t); %power [W]
Egen_f = int(fun_f,t,t_enter_f,t_exit_f); %energy [W.hr]

%aft 
t_enter_a = T/2; %time aft panel enters light [s]
t_exit_a = T - tEclipse/2; %time aft panel exits light [s]

fun_a = Fsun*APanel*effEoL*-sin(w*t); %power [W]
Egen_a =  int(fun_a,t,t_enter_a,t_exit_a); %energy [W.hr]

EgenBody = Egen_f + Egen_a;
double(EgenBody)

%% Power generated by panels [W]
pGen = Fsun*effPanel*(1 - effTemp*(Tsol - 28))*(1 - effTime)^opTime*APanel;
EgenTracking = pGen*tSun*orbits; % Energy generated by tracking panels per day [J]
