...
syms t
eGenBody = double(Fsun*APanel*effEoL*( 2*int(sin(w*t),t, thetaEclipse/w, pi/w)));
...
function power = pGenBody1(theta)
	if(theta > thetaEclipse && theta < wrapTo2Pi(-thetaEclipse))
		power = Fsun*APanel*effEoL*2*sin(w*t);
	else
		power = 0;
	end
end