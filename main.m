...
pGenTracking = 2*Fsun*effEoL*APanel*.8; % [W] power generated by the panels 
...
pModeIdle = (pCPU(1) + rxPower + pSens_sby + pACS(1)); %*(1+margin);
% Comms only power consumed [W]
pModeComms = (pACS(2) + pCPU(1) + pSens_sby + rxTxPower); %*(1+margin);
% Power generation
...
syms dRate
linkEqFixed = EbNoMin == maxTxPowerOut - Ls(EMdist,fTx) + G(diaG,fTx)...
	+ G(diaScFixed,fTx) + 228.6 - Ts - 10*log10(dRate) - Ll;
linkEqDep = EbNoMin == maxTxPowerOut - Ls(EMdist,fTx) + G(diaG,fTx)...
...
dRateMaxDep = double(solve(linkEqDep,dRate));  % Max data rate that meets link budget [bps]
...
% Fixed Panels
nfixed = .25; % number orbits collecting science 
ntx = nfixed * ((T*100000) / (tSun*dRateMaxfixed)); % orbits transmiting
ngen = floor(orbits) - ceil(ntx) - nfixed; % orbits generating power for fixed panels
% tau = (tSun/T); % fraction of period spent in the sun
% ttx = (100000)/(dRateMax); % [s] time for data transfer
Esci_fixed = nfixed * (PgenFixed45*int(cos(w*t),t,0,tSun) - T*pModeSci*(.8)); %  energy used in one orbit for data collection 
Etx_fixed = -ntx*tSun*pModeComms; % energy for transmitting data 
Egen_fixed = ngen * (PgenFixed45*tSun  - T*pModeIdle); % energy generated by panels  

Eday_fixed = double( (Esci_fixed + Etx_fixed + Egen_fixed)/3600 ); % [W.hr] net energy gained/loss

% Tracking Panels 
ntrack = 1.25; % number of orbits collecting science 
