...
% Power Consumption 
figure(2)
pSciTrack24 = pTime(pModeSci,T/60,0,24*60, 1:3); % power consumed during science collection 
...
xlabel('Time (min)'), ylabel('Power (W)')
legend('Science','Transmiting','Idle','Power Generation')
...
% Power Consumption
pSciFixed24 = pTime(pModeSci,T/60,0,24*60, 1:2); % power consumed during science collection 
pCommFixed24 = pTime(pModeComms,T/60,[tEclipse(1) tEclipse(1)+ttxFixed]/60,24*60, 10); % power consumed during communication
pIdleFixed24 = pTime(pModeIdle,T/60,tEclipse/60,24*60,[3:9 11 12],true); % power consumed during idle 
pPowerFixed24 = pTime(pModeGen,T/60,tEclipse/60,24*60,[3:9 11 12]); % power consumed during power generation 

figure(4)
plot(pSciFixed24(:,1) , [pSciFixed24(:,2),pCommFixed24(:,2),pIdleFixed24(:,2),pPowerFixed24(:,2)], 'lineWidth', 2)
grid on
title('Power generated by Fixed Panels (24 hrs)')
xlabel('Time (min)'), ylabel('Power (W)')
legend('Science','Transmiting','Idle','Power Generation')
