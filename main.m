...
PgenFixed45 = cosd(45)*PgenFixed(0); % Power generated by fixe panels (oriented 45deg) [W]
...
grid on
title('Power Consumption for High-Cost Spacecraft (24 hrs)')
xlabel('Time (min)'), ylabel('Power (W)')
legend('Science','Transmiting','Idle','Power Generation')

% -- Low-cost plots
% Power Generation
pGenFixed24 = pTime(PgenFixed,T,tEclipse,24*3600,[1:9 11:ceil(orbits)]);
figure(3)
plot(pGenFixed24(:,1), pGenFixed24(:,2), 'lineWidth', 2)
grid on
title('Power Generation for Low-Cost Spacecraft (24 hrs)')
xlabel('Time (min)'), ylabel('Power (W)')
...
pIdleFixed24 = pTime(pModeIdle,T/60,0,24*60,[3:9 11 12]); % power consumed during idle

figure(4)
plot(pSciFixed24(:,1) , [pSciFixed24(:,2),pCommFixed24(:,2),pIdleFixed24(:,2)], 'lineWidth', 2)
grid on
title('Power Consumption for Low-Cost Spacecraft (24 hrs)')
xlabel('Time (min)'), ylabel('Power (W)')
legend('Science','Transmiting','Idle')